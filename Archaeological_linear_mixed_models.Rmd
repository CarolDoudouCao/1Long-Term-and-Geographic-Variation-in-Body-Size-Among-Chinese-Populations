...
```{r, include=FALSE}
# Display the plot
print(plot_full_fxl_coefficients)
```

```{r, include = FALSE}
# Save the plot with custom dimensions
ggsave("fxl_coefficient_plot.png", plot = plot_full_fxl_coefficients, width = 12, height = 10)
```
...
```{r, include=FALSE}
# Display the plot
print(plot_full_txl_coefficients)
```


```{r, include = FALSE}
# Save the plot with specified dimensions
ggsave("txl_coefficient_plot.png", plot = plot_full_txl_coefficients, width = 12, height = 10)
```
...
#### HXL
# Tidy the model outputs and add model labels
tidy_male_full_hxl <- tidy(lme_male_hxl_full1, effects = "fixed") %>%
  mutate(model = "Male")
tidy_female_full_hxl <- tidy(lme_female_hxl_full1, effects = "fixed") %>%
  mutate(model = "Female")

# Combine male and female outputs
tidy_combined_full_hxl <- bind_rows(tidy_male_full_hxl, tidy_female_full_hxl) %>%
  filter(term != "(Intercept)") %>% # Remove intercept
  mutate(
    sig = ifelse(p.value < 0.05, "Sig", "Not Sig"),         # Add significance flag
    ci_low = estimate - 1.96 * std.error,                  # Compute lower CI
...
# Reorder terms for plotting
tidy_combined_full_hxl$term <- factor(tidy_combined_full_hxl$term, levels = unique(tidy_combined_full_hxl$term))

# Create the plot
plot_full_hxl_coefficients <- ggplot(tidy_combined_full_hxl, aes(y = term, x = estimate, fill = interaction(model, sig))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(aes(xmin = ci_low, xmax = ci_high), 
                position = position_dodge(0.7), width = 0.2) +
  labs(
    y = "Predictor Variables", 
    x = "Coefficient Estimate (with 95% Confidence Interval)", 
    title = "Fixed Effects Coefficient Estimates for Male and Female Models (HXL)"
  ) +
  # Custom fill scale for significance and model interaction
  scale_fill_manual(values = c(
    "Male.Sig" = "blue", 
...
# Display the plot
print(plot_full_hxl_coefficients)
```

```{r, include = FALSE}
# Save the plot with specified dimensions
ggsave("hxl_coefficient_plot.png", plot = plot_full_hxl_coefficients, width = 12, height = 10)
```
...
#### RXL
# Tidy the model outputs and add model labels
tidy_male_full_rxl <- tidy(lme_male_rxl_full1, effects = "fixed") %>%
  mutate(model = "Male")
tidy_female_full_rxl <- tidy(lme_female_rxl_full1, effects = "fixed") %>%
  mutate(model = "Female")

# Combine male and female outputs
tidy_combined_full_rxl <- bind_rows(tidy_male_full_rxl, tidy_female_full_rxl) %>%
  filter(term != "(Intercept)") %>% # Remove intercept
  mutate(
    sig = ifelse(p.value < 0.05, "Sig", "Not Sig"),         # Add significance flag
    ci_low = estimate - 1.96 * std.error,                  # Compute lower CI
...
# Reorder terms for plotting
tidy_combined_full_rxl$term <- factor(tidy_combined_full_rxl$term, levels = unique(tidy_combined_full_rxl$term))

# Create the plot
plot_full_rxl_coefficients <- ggplot(tidy_combined_full_rxl, aes(y = term, x = estimate, fill = interaction(model, sig))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(aes(xmin = ci_low, xmax = ci_high), 
                position = position_dodge(0.7), width = 0.2) +
  labs(
    y = "Predictor Variables", 
    x = "Coefficient Estimate (with 95% Confidence Interval)", 
    title = "Fixed Effects Coefficient Estimates for Male and Female Models (RXL)"
  ) +
  scale_fill_manual(values = c(
    "Male.Sig" = "blue", 
    "Female.Sig" = "red", 
...
# Display the plot
print(plot_full_rxl_coefficients)
```

```{r, include = FALSE}
# Save the plot
ggsave("rxl_coefficient_plot.png", plot = plot_full_rxl_coefficients, width = 12, height = 10)
```
...
```{r, include=FALSE}
# Display the plot
print(plot_full_fhd_coefficients)
```

```{r, include = FALSE}
# Save the plot
ggsave("fhd_coefficient_plot.png", plot = plot_full_fhd_coefficients, width = 12, height = 10)
```
